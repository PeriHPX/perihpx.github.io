...
\[x(\xi, \eta, \zeta) = \sum_{i=1}^n N^0_i(\xi, \eta, \zeta) v^i_x, \quad
y(\xi, \eta, \zeta) = \sum_{i=1}^n N^0_i(\xi, \eta, \zeta) v^i_y, \quad z
(\xi, \eta, \zeta) = \sum_{i=1}^n N^0_i(\xi, \eta, \zeta) v^i_z \]
\pagebreak
...
$ \frac{\partial
N_1}{\partial x}, \frac{\partial N_2}{\partial x}$
\pagebreak
...
\[N^0_1(\xi, \eta) = \frac{(1- \xi)(1 - \eta)}{4}, \quad N^0_2(\xi, \eta) =
\frac{(1+ \xi)(1 - \eta)}{4}, \]
\pagebreak
...
\[ f_1(x,y) =  a \exp(-\frac{(x-x_c)^2 + (y-y_c)^2}{\beta}) d_1,
...
\[ E(t) = \int_{\Gamma(t)} \left[ \frac{1}{2} \rho
|\dot{u}(t)|^2 dt +
\bar{W}(x; u(t)) \right] v\cdot n dx \]
\pagebreak

\[ \frac{2}{\epsilon |B_\epsilon(0)|}
\int_{A^c(t)} \int_{A(t) \cap B_\epsilon(x)} \partial_S W(S(y,x;
u(t))) \frac{y-x}{|y-x|} \cdot (\dot{u}(x,t) + \dot{u}(y,t)) dy dx \]
\pagebreak
